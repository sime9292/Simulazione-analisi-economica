/* v46 - Test Environment seeds offer, analysis, confirmed offer lines and shared billing plan. Billing stays manual. */
(function(){
  ...
  const CASES={
    '26_022pe01':{
      ...
      billingPlan:[
        {id:'26_022pe01:plan:deposit',baseType:'offer',eventLabel:'Acconto',valueType:'percent',value:10,trigger:'confirmation'},
        {id:'26_022pe01:plan:pdc-delivery',baseType:'line',basePhase:'esecutivo',eventLabel:'Saldo Progetto PDC',valueType:'percent',value:90,trigger:'delivery',activityPhase:'esecutivo',activityName:'Progetto impianti per PDC'}
      ]
    }
  };
  ...
  function enforceExactOfferLines(c){
    ...
    c.lines.forEach(line=>{
      const row=[...root.querySelectorAll('.offer-line-row')].find(r=>(r.dataset.phase||r.querySelector('.offer-line-phase-select')?.value||'')===line.phase);
      if(!row)return;
      row.hidden=false;
  ...
  async function loadAnalysis(){
    if(busy)return;busy=true;lastError='';renderBar();try{
      ...
      window.DABSTER_BILLING_PLAN_V46?.reset?.();
  ...
  async function confirmAndCreateLines(){
    if(busy||stage<1)return;busy=true;lastError='';renderBar();try{
      ...
      if(!check.ok){lastError=check.message;console.error('[Dabster Test v46]',lastError);stage=1;sessionStorage.setItem(STAGE_KEY,'1');scrollTo(document.getElementById('offerLinesSection'));return;}
      stage=2;sessionStorage.setItem(STAGE_KEY,'2');billingPrepared=false;await patchFlow();window.DABSTER_BILLING_PLAN_V46?.refresh?.();scrollTo(document.getElementById('offerLinesSection'));
    }finally{busy=false;renderBar();}
  }

  async function loadBillingPlan(){
    if(busy||stage<2)return;busy=true;lastError='';renderBar();try{
      const api=await waitFor(()=>window.DABSTER_BILLING_PLAN_V46?.seed&&window.DABSTER_BILLING_PLAN_V46,260,45);
      if(!api){lastError='Piano di fatturazione non disponibile.';return;}
      const result=api.seed(currentCase().billingPlan,{replace:true});
      if(result.incomplete){lastError=`Piano caricato con ${result.incomplete} regola/e incomplete.`;stage=2;sessionStorage.setItem(STAGE_KEY,'2');return;}
