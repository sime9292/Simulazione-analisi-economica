/* v59 - compact hours + centered economics + managerial dimensioning + planning separation */
(function(){
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      #tab-analisi #analysisSubtabImpianti .economic-table .computed-cell,
      #tab-analisi #analysisSubtabImpianti .economic-table .total-value{justify-content:center!important;text-align:center!important}
      ...
      /* Sticky economics is a summary area; planning below is a separate operational area. */
      ...
      html body #tab-analisi #analysisSubtabImpianti>.flat-planning{
        position:relative!important;margin-top:17px!important;padding-top:13px!important;
        border-top:6px solid #edf1f3!important;
      }
      html body #tab-analisi #analysisSubtabImpianti>.flat-planning::before{
        content:'';position:absolute;left:0;right:0;top:-7px;height:1px;background:#dce3e7;
      }
      html body #tab-analisi #analysisSubtabImpianti>.flat-planning .workload-toolbar{
        padding-top:3px!important;margin-bottom:5px!important;
      }
      ...
      html body #tab-analisi #analysisSubtabImpianti #planningPhaseTabs .kanban-phase-tab{
        min-width:112px!important;padding-left:8px!important;padding-right:8px!important;font-size:8.9px!important;
      }
      ...
      #phaseWorkloadSection .assignment-head,#phaseWorkloadSection .assignment-row{grid-template-columns:minmax(135px,1fr) 82px 72px 96px 24px!important}
  ...
  function setDirectLabel(cell,label){
    if(!cell)return;let node=[...cell.childNodes].find(n=>n.nodeType===Node.TEXT_NODE&&String(n.textContent||'').trim());
  ...
  function cleanEconomicUnits(){
    const head=document.querySelector('#tab-analisi #analysisSubtabImpianti .economic-table .economic-head');if(!head||head.children.length<4)return;
    setDirectLabel(head.children[0],'FASE');setDirectLabel(head.children[1],'PROPOSTA €');
    const trade=head.children[2].querySelector('.trade-inline>span');if(trade)trade.textContent='TRATTATIVA €';setDirectLabel(head.children[3],'COSTI €');
  }

  function centerKpis(){
    document.querySelectorAll('#tab-analisi #analysisSubtabImpianti .economic-kpis .kpi').forEach(kpi=>{
      kpi.style.setProperty('justify-items','center','important');kpi.style.setProperty('align-items','center','important');kpi.style.setProperty('align-content','center','important');kpi.style.setProperty('text-align','center','important');
      kpi.querySelectorAll('.kpi-label,.kpi-value,.kpi-sub').forEach(el=>{
        el.style.setProperty('display','block','important');el.style.setProperty('width','100%','important');el.style.setProperty('text-align','center','important');
        el.style.setProperty('justify-self','center','important');el.style.setProperty('align-self','center','important');el.style.setProperty('margin-left','auto','important');el.style.setProperty('margin-right','auto','important');
      });
    });
  }
  ...
  function syncKnob(input,knob){
    ...
    knob.style.setProperty('--scrub-angle',angle+'deg');knob.setAttribute('aria-label',`Ore ${value.toLocaleString('it-IT',{maximumFractionDigits:1})}. Trascina orizzontalmente per modificare.`);
  }
  ...
  function enhanceHoursInput(input){
    ...
    const knob=document.createElement('button');knob.type='button';knob.className='hours-scrubber-knob';knob.title='Trascina a sinistra/destra per diminuire/aumentare le ore';wrap.insertBefore(knob,input);
    ...
    const setValue=value=>{const next=Math.max(0,Math.round(value/step)*step),rounded=Math.round(next*10)/10;if(numeric(input.value)===rounded)return;input.value=Number.isInteger(rounded)?String(rounded):rounded.toFixed(1);lastValue=rounded;input.dispatchEvent(new Event('input',{bubbles:true}));syncKnob(input,knob);};
    knob.addEventListener('pointerdown',e=>{if(e.button!==undefined&&e.button!==0)return;e.preventDefault();pointerId=e.pointerId;startX=e.clientX;startValue=numeric(input.value);lastValue=startValue;moved=false;knob.setPointerCapture?.(pointerId);knob.classList.add('scrubbing');document.body.classList.add('hours-scrubbing');});
    knob.addEventListener('pointermove',e=>{if(pointerId===null||e.pointerId!==pointerId)return;const dx=e.clientX-startX;if(Math.abs(dx)>2)moved=true;setValue(startValue+Math.round(dx/3)*step);});
    const finish=e=>{if(pointerId===null||(e.pointerId!==undefined&&e.pointerId!==pointerId))return;try{knob.releasePointerCapture?.(pointerId);}catch(_e){}pointerId=null;knob.classList.remove('scrubbing');document.body.classList.remove('hours-scrubbing');if(lastValue!==null)input.dispatchEvent(new Event('change',{bubbles:true}));if(!moved)setTimeout(()=>{input.focus();input.select?.();},0);};
    knob.addEventListener('pointerup',finish);knob.addEventListener('pointercancel',finish);input.addEventListener('input',()=>syncKnob(input,knob));input.addEventListener('change',()=>syncKnob(input,knob));syncKnob(input,knob);
  }

  function enhanceAll(root=document){root.querySelectorAll?.('#phaseWorkloadSection .assignment-hours').forEach(enhanceHoursInput);cleanEconomicUnits();centerKpis();polishDimensionLabels();}
  ...
  function install(attempt=0){
    ...
    new MutationObserver(mutations=>{mutations.forEach(m=>m.addedNodes.forEach(node=>{if(!(node instanceof HTMLElement))return;if(node.matches?.('.assignment-hours'))enhanceHoursInput(node);node.querySelectorAll?.('.assignment-hours').forEach(enhanceHoursInput);}));}).observe(workload,{childList:true,subtree:true});
    document.addEventListener('click',e=>{if(e.target.closest('#prefillDemoData,#clearDemoData,#dimTransfer,.analysis-subtab'))setTimeout(()=>enhanceAll(),180);},true);
