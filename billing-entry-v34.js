/* v47 entry - Shared lazy loader for dashboard/manual invoice + native Billing Plan prefill. */
(function(){
  let loadingPromise=null,loaded=!!(window.DABSTER_BILLING_V40||window.DABSTER_BILLING_V39);
  ...
  async function ensureInvoiceSource(){
    ...
    const source=await ensureModule('script[data-plan-invoice-source-v56]','billing-plan-source-v52.js?v=56',()=>window.DABSTER_BILLING_PLAN_SOURCE_V56,'planInvoiceSourceV56');source?.refresh?.();setTimeout(()=>source?.refresh?.(),60);return source;
  }
  function currentApi(){return window.DABSTER_BILLING_V40||window.DABSTER_BILLING_V39||null;}
  function openTarget(target){const api=currentApi();if(!api)return null;if(target==='invoice'){api.showInvoice?.();setTimeout(()=>ensureInvoiceSource(),0);}else api.showDashboard?.();return api;}
  function loadWorkspace(target='dashboard'){
    ...
    if(currentApi()){loaded=true;return Promise.resolve(openTarget(target));}
    ...
    loadingPromise=new Promise((resolve,reject)=>{
      ...
      if(existing){const wait=()=>{if(currentApi()){loaded=true;resolve(currentApi());}else setTimeout(wait,30);};wait();return;}
      const script=document.createElement('script');script.src='billing-workspace-v39.js?v=40';script.dataset.cleanBillingWorkspace='1';
      script.onload=()=>{loaded=true;if(!document.querySelector('script[data-billing-dashboard-totals-v40]')){const totals=document.createElement('script');totals.src='billing-dashboard-totals-v40.js?v=40';totals.dataset.billingDashboardTotalsV40='1';document.head.appendChild(totals);}resolve(currentApi());};
