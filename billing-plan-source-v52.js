...
(function(){
  ...
  function applyMode(body){
    const source=body.querySelector('#ni39Source'),panel=body.querySelector('.bp52-plan-panel');if(!source||!panel)return;
    const plan=mode==='plan';source.hidden=plan;panel.hidden=!plan;
    body.querySelectorAll('[data-bp52-mode]').forEach(b=>b.classList.toggle('active',b.dataset.bp52Mode===mode));
