/* v50 entry - Shared lazy loader for dashboard/manual invoice + trigger-aware Billing Plan prefill. */
(function(){
  ...
  function activeContext(){
    ...
    if(!snap?.offer?.code||!Array.isArray(snap.lines)||!snap.lines.length)return null;
    const amount=Number(snap.offer.amount||snap.lines.reduce((s,x)=>s+Number(x.amount||0),0));
    if(amount<=0)return null;
    return snap;
  }
  async function waitActiveContext(){return waitFor(activeContext,220,30);}
  ...
  function showUnavailable(){
    console.warn('[Dabster] Fatturazione non disponibile: serve un’offerta confermata con Righe Offerta.');
    const bar=document.getElementById('dabsterEnvironmentBar');
    if(bar){
      let note=bar.querySelector('.billing-context-note');
      if(!note){note=document.createElement('span');note.className='td50-error billing-context-note';bar.querySelector('.td50-row')?.appendChild(note);}
      note.textContent='Conferma prima l’offerta e crea le Righe Offerta per aprire Fatturazione.';
  ...
  async function loadWorkspace(target='dashboard'){
    const ctx=activeContext()||await waitActiveContext();
    if(!ctx){showUnavailable();return null;}
    ensureCleanModel();
    if(target==='invoice')history.replaceState(null,'','#nuova-fattura');else history.replaceState(null,'','#dashboard-fatturazione');
    if(nativeApi())return openTarget(target);
