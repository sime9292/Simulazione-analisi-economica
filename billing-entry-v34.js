/* v45 entry - Shared lazy loader for dashboard/manual invoice + Billing Plan prefill below Offer Lines. */
(function(){
  ...
  async function ensureInvoiceSource(){
    await ensureModule('script[data-plan-invoice-v53]','billing-plan-invoice-v51.js?v=53',()=>window.DABSTER_PLAN_TO_INVOICE_V53,'planInvoiceV53');
  ...
  function install(attempt=0){
    ...
    if(btn.dataset.billingEntryReady!=='45'){
      btn.dataset.billingEntryReady='45';btn.addEventListener('click',e=>{if(window.DABSTER_BILLING_V39)return;e.preventDefault();e.stopImmediatePropagation();loadWorkspace('dashboard');},true);
    }
