/* v77 loader: full established UI, authoritative Kanban -> Billing Trigger runtime. */
(function(){
  ...
  function loadTriggerNotify(){
    if(document.querySelector('script[data-trigger-notify-v77]'))return;
    const notify=document.createElement('script');notify.src='trigger-notify-v77.js?v=77';notify.dataset.triggerNotifyV77='1';
    notify.onerror=()=>console.error('[Dabster] Errore notifiche Trigger v77');document.head.appendChild(notify);
  }
  function waitTriggerReady(attempt=0){
    if(window.DABSTER_BILLING_TRIGGER_V58){loadTriggerNotify();return;}
    if(attempt<240)setTimeout(()=>waitTriggerReady(attempt+1),25);
    else console.error('[Dabster] Trigger v77 non disponibile dopo il caricamento.');
  }
  function loadBillingTrigger(){
    if(document.querySelector('script[data-billing-trigger-runtime-v77]')){waitTriggerReady();return;}
    const trigger=document.createElement('script');trigger.src='billing-trigger-runtime-v77.js?v=77';trigger.dataset.billingTriggerRuntimeV77='1';
    trigger.onload=()=>waitTriggerReady();
    trigger.onerror=()=>console.error('[Dabster] Errore caricamento Trigger runtime v77');document.head.appendChild(trigger);
  }
