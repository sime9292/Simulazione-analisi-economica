/* v46 entry - Shared lazy loader for dashboard/manual invoice + Billing Plan prefill below Offer Lines. */
(function(){
  ...
  async function ensureInvoiceSource(){
    await ensureModule('script[data-plan-invoice-v54]','billing-plan-invoice-v51.js?v=54',()=>window.DABSTER_PLAN_TO_INVOICE_V54,'planInvoiceV54');
    const source=await ensureModule('script[data-plan-invoice-source-v56]','billing-plan-source-v52.js?v=56',()=>window.DABSTER_BILLING_PLAN_SOURCE_V56,'planInvoiceSourceV56');
  ...
  window.DABSTER_BILLING_ENTRY_V46=api;
  window.DABSTER_BILLING_ENTRY_V45=api;
  window.DABSTER_BILLING_ENTRY_V44=api;
  ...
  function install(attempt=0){
    ...
    if(btn.dataset.billingEntryReady!=='46'){
      btn.dataset.billingEntryReady='46';btn.addEventListener('click',e=>{if(window.DABSTER_BILLING_V39)return;e.preventDefault();e.stopImmediatePropagation();loadWorkspace('dashboard');},true);
    }
